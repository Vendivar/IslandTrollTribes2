...
function resist_all_upgrade( keys )
...
function resist_all_charge( keys )
	...
	local modifierName = "modifier_resist_all_counter_datadriven"
	...
	caster.resist_all_charges = maximum_charges
	caster.start_charge = false
	caster.resist_all_cooldown = 0.0
	...
	Timers:CreateTimer( function()
			-- Restore charge
			if caster.start_charge and caster.resist_all_charges < maximum_charges then
				-- Calculate stacks
				local next_charge = caster.resist_all_charges + 1
				...
				if next_charge ~= maximum_charges then
					...
					resist_all_start_cooldown( caster, charge_replenish_time )
				else
					ability:ApplyDataDrivenModifier( caster, caster, modifierName, {} )
					caster.start_charge = false
				end
				...
				-- Update stack
				caster.resist_all_charges = next_charge
			end
			
			-- Check if max is reached then check every 0.5 seconds if the charge is used
			if caster.resist_all_charges ~= maximum_charges then
...
function resist_all_start_cooldown( caster, charge_replenish_time )
	caster.resist_all_cooldown = charge_replenish_time
	Timers:CreateTimer( function()
			local current_cooldown = caster.resist_all_cooldown - 0.1
			if current_cooldown > 0.1 then
				caster.resist_all_cooldown = current_cooldown
				return 0.1
			else
				return nil
			end
...
function resistall_add( keys )
	-- Check condition
	if keys.caster.resist_all_charges < 1 then
		return
	else
		keys.caster.resist_all_charges = keys.caster.resist_all_charges - 1
	end
	...
	local ability = keys.ability
	local modifierCounterName = "modifier_resist_all_counter_datadriven"
	local modifierCounterCooldownName = "modifier_resist_all_counter_cooldown_datadriven"
	local maximum_charges = ability:GetLevelSpecialValueFor( "max_charges", ( ability:GetLevel() - 1 ) )
	local charge_replenish_time = ability:GetLevelSpecialValueFor( "charge_restore_time", ( ability:GetLevel() - 1 ) )
	...
	if not caster.resist_all_entities then caster.resist_all_entities = {} end
	
	caster.resist_all_entities[ dummy:entindex() ] = target
	
	-- Check if it should start cooldown timer
	if caster.resist_all_charges + 1 == maximum_charges then
	...
	caster:SetModifierStackCount( modifierCounterName, caster, caster.resist_all_charges )
	
	-- Reduce the charges in counter
	if caster.resist_all_charges > 0 then
		ability:EndCooldown()
	else
		ability:StartCooldown( caster.resist_all_cooldown )
	end
...
function resist_all_activate( keys )
	local caster = keys.caster	
	local target = keys.target_points[1]
	...
	for k, v in pairs( caster.resist_all_entities ) do
		local dummy = EntIndexToHScript( k )
		if dummy == nil or not dummy:IsAlive() then
			caster.resist_all_entities[k] = nil
		else
	...
	if index == -1 then
		...
		caster:RemoveModifierByName( "modifier_activate_resist_all_buff_datadriven" )
		caster:RemoveModifierByName( "modifier_activate_resist_all_destroy_tree_datadriven" )
		return
	end
	...
	-- Set up variables
	local destination = caster.resist_all_entities[index]
	local forwardVec = ( destination - caster:GetAbsOrigin() ):Normalized()
	caster.resist_all_entities[index] = nil
	...
	Timers:CreateTimer( function()
			...
			if ( newLoc - destination ):Length2D() < 50 or counter >= timer_upper_bound then
				...
				caster:RemoveModifierByName( "modifier_resist_all_counter_cooldown_datadriven" )
				if dummy ~= nil then
					dummy:RemoveSelf()
				end
