    ...
    if findStack and maxStacks then
      ...
      if nameTable then
        ...
        for itemid, nameslot in pairs(nameTable) do
          if nameslot < lowestSlot then
            local item = self:GetItemInSlot(nameslot)
            if item then                                      -- This for loop was kinda meaningless before...
              if item:GetCurrentCharges() < maxStacks then    -- It always went through the slots and returned the first one
                lowestSlot = nameslot                         -- Now it only returns a slot that doesn't have maxStacks yet.
                lowestItem = item                             -- Not sure if Line 2345 should get the same treatment.
              end
            else
              nameTable[itemid] = nil
            end
