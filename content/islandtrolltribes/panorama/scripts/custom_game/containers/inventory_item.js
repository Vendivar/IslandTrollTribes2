...
function UpdateItem()
{
	...
	var mana = Entities.GetMana(m_QueryUnit);
	...
	$.GetContextPanel().SetHasClass( "low_mana", (Abilities.GetManaCost( m_Item ) > Entities.GetMana(m_QueryUnit)) && (m_QueryUnit !== -1));
...
function OnDragDrop( panelId, draggedPanel )
{
	...
	var container = null;
	if (dropTarget.data().GetContainer)
		container = dropTarget.data().GetContainer();
	...
	var action = PlayerTables.GetTableValue(m_contString, "OnDragFrom");
	if (action !== 0){
...
(function()
{
	...
	$.GetContextPanel().data().GetContainer = GetContainer;
